...
func (h *PaymentHandler) GetPlans(c *gin.Context) {
	...
	for _, p := range plans {
		result = append(result, planWithPlatform{
			ID: int64(p.ID), GroupID: p.GroupID, GroupPlatform: disguiseUserVisiblePlatform(platformMap[p.GroupID]),
...
func (h *PaymentHandler) GetCheckoutInfo(c *gin.Context) {
	...
	for _, p := range plans {
		...
		planList = append(planList, checkoutPlan{
			...
			GroupPlatform: disguiseUserVisiblePlatform(gi.Platform), GroupName: gi.Name,
